 ...
 /*----------------------------
  ...
  /*-------------------------
    ...
    /*---------------------------
       HEADER SEARCH BAR
    ------------------------------ */
    $('#media-header-search').on('submit', function(e){
        e.preventDefault();

        var form = $(this);
        var searchTerm = form.find('input').val();
        var categorySlug = form.find('select').val();

        if(!searchTerm && !categorySlug)
        {
            showSimpleMessage("Attention", "Please provide a search term or a category for this search", "error");
            return false;
        }

        window.location = `events?category=${categorySlug}&search=${encodeURI(searchTerm)}`;
    });
    ...
    $('#mc-embedded-subscribe-form').on('submit', function(e){
        ...
        var form = $(this);
        var email = form.find('#footer_email_address').val();

        blockUI();

        if(!validateEmail(email))
        {
            form.find('#footer_email_address').focus();
            unblockUI();
    ...
    $(window).on('load', function(event) {
        ...
        $.ajax({
            ...
            success:function(response)
            {
                if(response.error == false)
                {
                    var categories = response.result.mediaEventCategories;
                    var categoriesHTML = '<option value="">All categories</option>';

                    for(var i = 0; i < categories.length; i++)
                    {
                        categoriesHTML += `
                            <option value="${categories[i].media_event_category_slug}">
                                ${categories[i].media_event_category}
                            </option>
                        `;
                    }

                    $('header form select').html(categoriesHTML);
                    $('header form select').niceSelect('update');
                }
            }
        });
