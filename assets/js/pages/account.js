$(function () {
	...
	$(document).ready(function($) {
        ...
        //toggle passwords
        togglePasswords();
        //upload images for applicants only
        newMedia();
        //load my uploaded media
        loadMyUploadedMedia();
        //view my uploaded media
        myUploadedMediaActions();
    });
            ...
            swal({
                ...
                if (result.value) 
                {
                    ...
                    if(avatar)
                    {
                        ...
                        if($("#avatar").get(0).files[0].size > (1024 * 1000)) 
                        {
    ...
    //internal function to add new media
    function newMedia() 
    {
        $('#image-upload').on('submit', function(e){
            ...
            swal({
                ...
                if (result.value) 
                {
                    ...
                    var media = form.find("#media"); //media from form
                    var validFileExtensions = ['jpeg','jpg', 'png'];
                    var table = $('#uploaded-media').DataTable();
                    var fields = form.find('input.required, select.required');  
                    ...
                    for(var i = 0; i < media.get(0).files.length; i++)
                    {
                        var filename = media.get(0).files[i].name;
                        ...
                        if($.inArray(extension, validFileExtensions) == -1)
                        {
                            ...
                            unblockUI();
                            media.focus();
                            showSimpleMessage("Attention", "Media must be jpeg, jpg or png formatted", "error");
                            return false;
                        }
                        
                        /*if(media.get(0).files[i].size > (1024 * 200)) 
                        {
                            //pop image is more than 200KB
                            unblockUI();
                            pops.focus();
                            showSimpleMessage("Attention", "Image files must not be more than 200KB in size", "error");
                            return false;
                        } */
                    }
    ...
    //load my uploaded media
    function loadMyUploadedMedia()
    {
        var table = $('#uploaded-media');
        var userID = payloadClaim(token, 'user_id');
        ...
        table.DataTable({
            ...
            ajax: {
                type: 'GET',
                url: API_URL_ROOT+`/media/data-table/fetch?user_id=${userID}`,
                dataType: 'json',
                headers:{'x-access-token':token},
                complete: function()
                {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta)
                    {
                        return `<div class="avatar avatar-xl"><img alt="avatar" src="${data}" class="rounded img-fluid" width="50" height="50" /></div>`
                    }
                },
                {
                ...
                },
                {
                    ...
                    render:function(data, type, row, meta)
                    {
                        var actions = `
                            ...
                            <a href="javascript:void(0);" 
                                data-id="${data}"
                                class="btn btn-link font-18 text-muted btn-sm btn-view" 
                                data-toggle="modal" 
                            ...
                            <a href="javascript:void(0);" 
                                data-id="${data}"
                                class="btn btn-link font-18 text-muted btn-sm btn-delete" 
                                title="Delete Media"> 
    ...
    function myUploadedMediaActions()
    {
        $('#uploaded-media').on('click', '.btn-view', function(){
            ...
            $.ajax({
                ...
                success: function(response)
                {
                    if(response.error == false)
                    {
                        ...
                        $('#messageModal').find('.message-body').html(`<img src="${media.media_url}" width="100%" height="100%" class="img-responsive"/>`);
        ...
        $('#uploaded-media').on('click', '.btn-delete', function(){
            var mediaID = $(this).attr('data-id');
